...
### Column and soffit systems declare what they form, and the per-kind rating replaces the wall-pressure invariant wholesale

The datasheet research on the six unseeded registrations showed four of them are not wall-panel lines — SRS and QUATTRO form columns, Skydeck and MULTIFLEX form slab soffits — and the registry currently has one shape for a system: rectangular wall panels against `pressure.wallsKnM2`. Seeding them would have forced column and soffit data through a wall-pressure field, which is the exact fiction 4.1 exists to refuse. So the decision is a capability, not a workaround: **a `FormworkSystem` declares which element kinds it supports** (walls, columns, slabs), the layout refuses a system on a kind it does not support with both names (same `rejected` channel as `system-unseeded`, new reason `system-kind`), and a seeded system must carry the rating each supported kind needs — a wall system a wall pressure, a column system a column rating (a curve of pressure against pour height for a circular form, or a flat `columnsKnM2`), a soffit system its own capacities (panel/beam/prop table) rather than any pressure at all.

**The 4.1 invariant generalises from "every seeded panel carries a wall pressure" to "every supported kind of every seeded system carries its rating".** That is what makes a column-only system with a column rating *seeded* instead of unseeded — the previous invariant would have forced one or the other, and forcing it unseeded would have refused the one element kind it actually forms.

*Alternative rejected — a `systemShape` discriminator that keeps one rating field:* a `'column'` shape still needs a rating, and a `'soffit'` shape needs capacities that are not pressures at all, so the discriminator ends up parameterising the rating by kind anyway — which is the capability, minus the refusal a supported-kind check gives for free. And *rejected — keep them unseeded forever:* the refusal is the honest fallback but it is not a design; the capability is small (a supported-kinds list on the registration, a kind check in the refusal chain, per-kind rating fields) and it is the difference between a catalog that can hold the systems the market sells and one that can only hold wall panels. What is *not* decided here is the geometry: a circular column form's panel is curved, and how a radius reaches `buildFormwork` is a follow-on change this one deliberately does not scope.
