/**
 * 'electrical' sheet drawing provider — STUB. Returns null (the viewport prints
 * "electrical — nothing to draw") until the owning workstream lands it.
 *
...
export function buildElectricalDrawing(_nodes: NodeMap, _args: ProviderArgs): DrawingResult | null {
  return null
}
...
export function registerElectricalProvider(
  ...
  register('electrical', (nodes, args) => buildElectricalDrawing(nodes, args as unknown as ProviderArgs))
}
