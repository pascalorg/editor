...
export function applyMultiHeightMode(nodeIds: AnyNodeId[], next: 'storey' | 'custom', parametrics: ParametricDescriptor<AnyNode>) {
  const nodes = useScene.getState().nodes as Record<string, AnyNode>
  commitMultiNodeFields(nodeIds, (node) => {
    ...
    if (!isCustom) return {}
    return node.type === 'wall' ? wallFollowsLevelPatch(node, nodes) : { height: undefined }
  }, parametrics)
}
