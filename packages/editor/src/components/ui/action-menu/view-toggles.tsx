...
function UploadButton({ onError }: { onError: (message: string | null) => void }) {
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!(isScan || isImage)) {
        onError(t('viewer.uploadGlbOrImage'))
        return
      }
      ...
      if (isImage) {
        ...
        try {
          ...
          onError(t('viewer.couldNotAddGuideImage'))
        } finally {
      ...
      if (!uploadHandler) {
        onError(t('viewer.scanUploadUnavailable'))
        return
      }

      const projectId = window.location.pathname.split('/editor/')[1]?.split('/')[0]
      if (!projectId) {
        onError(t('viewer.openProjectBeforeUpload'))
        return
      }
  ...
  return (
    <>
      <button
        aria-label={t('viewer.uploadScanOrGuide')}
    ...
    <Popover onOpenChange={setIsOpen} open={isOpen}>
      <div className="flex items-center">
        ...
        <ActionButton
          ...
          label={t('viewer.guidesState', { state: showGuides ? t('viewer.visible') : t('viewer.hidden') })}
        ...
        <PopoverTrigger asChild>
          <button
            ...
            aria-label={t('viewer.guideImageSettings')}
            className={cn(
                  ...
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    >
                      ...
                      <p className="truncate font-medium text-foreground text-sm">
                        {guide.name || t('viewer.guideImageDefault', { index: index + 1 })}
                      </p>
                      {selectedReferenceId === guide.id && (
                    ...
                    <button
                      aria-label={t('viewer.deleteGuideImage')}
    ...
    <Popover onOpenChange={setIsOpen} open={isOpen}>
      <div className="flex items-center">
        ...
        <ActionButton
          ...
          label={t('viewer.scansState', { state: showScans ? t('viewer.visible') : t('viewer.hidden') })}
        ...
        <PopoverTrigger asChild>
          <button
            aria-expanded={isOpen}
            aria-label={t('viewer.scanSettings')}
            className={cn(
              'flex h-11 w-6 items-center justify-center rounded-r-lg transition-colors',
              showScans
                ? isOpen
                ...
                >
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    >
                      ...
                      />
                      <p className="truncate font-medium text-foreground text-sm">
                        {scan.name || t('viewer.scanDefault', { index: index + 1 })}
                      </p>
                      {selectedReferenceId === scan.id && (
                        <Check className="ml-auto h-3.5 w-3.5 shrink-0 text-foreground/80" />
                      )}
                    ...
                    <button
                      aria-label={t('viewer.deleteScan')}
                      className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md text-muted-foreground/50 opacity-0 transition-all hover:bg-destructive/10 hover:text-destructive group-hover/item:opacity-100"
                      onClick={(event) => {
                        event.stopPropagation()
                        deleteNode(scan.id)
    ...
    <Popover onOpenChange={setIsOpen} open={isOpen}>
      <div className="flex items-center">
        <ActionButton
          ...
          label={t('viewer.referencesState', { state: anyVisible ? t('viewer.visible') : t('viewer.hidden') })}
          onClick={toggleAll}
          size="icon"
          variant="ghost"
        >
        ...
        <PopoverTrigger asChild>
          <button
            aria-expanded={isOpen}
            aria-label={t('viewer.referenceSettings')}
            className={cn(
              'flex h-11 w-6 items-center justify-center rounded-r-lg transition-colors',
              anyVisible
                ? isOpen
    ...
    <Popover onOpenChange={setIsOpen} open={isOpen}>
      <div className="flex items-center">
        <ActionButton
          ...
          label={
            selectedLevelName && showReferenceFloor
              ? t('viewer.referenceFloorWithLevel', { level: selectedLevelName })
        ...
        <PopoverTrigger asChild>
          <button
            aria-expanded={isOpen}
            aria-label={t('viewer.referenceFloorSettings')}
            className={cn(
              'flex h-11 w-6 items-center justify-center rounded-r-lg transition-colors',
              showReferenceFloor && selectedLevel
                ? isOpen
      ...
      >
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <button
              aria-label={showReferenceFloor ? t('viewer.hideReferenceFloor') : t('viewer.showReferenceFloor')}
                ...
                {lowerLevels.map((level, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-muted-foreground">{t('viewer.belowCount', { count: index + 1 })}</span>
                    </button>
                  )
                })}
              </div>
