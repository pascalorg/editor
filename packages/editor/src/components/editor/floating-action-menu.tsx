  ...
  const handleDuplicate = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (duplicate) {
        ...
        } else if (duplicate.type === 'chimney') {
          // Chimney uses pure drag-to-place: NO node is inserted into
          // the scene until the user clicks a roof segment. The
          // `setMovingNode` call below hands the clone (with
          // `metadata.isNew = true` + no id) to `MoveChimneyTool`,
          // which calls `createNode` on the click that commits the
          // placement. Skipping the auto-create avoids the "duplicate
          // appears at +1 offset before drag" UX the other registry
          // kinds use.
