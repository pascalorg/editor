...
export function snapFenceDraftPoint(args: {
  ...
  // An axis lock or a typed dimension is authoritative and outranks every
  // magnetic target. Same contract and same resolver as
  ...
  if (start) {
    ...
    if (constrained) return [constrained[0], constrained[1]]
  }
