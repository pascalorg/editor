...
describe('resolveCustomMeshFaceSwitch', () => {
  test('ignores a single adjacent-face hit and accepts a repeated hit', () => {
    const first = resolveCustomMeshFaceSwitch('face-a', 'face-b', null)
    expect(first).toEqual({ accept: false, pendingFaceId: 'face-b' })

    const second = resolveCustomMeshFaceSwitch('face-a', 'face-b', first.pendingFaceId)
  ...
  test('clears a pending switch when the pointer returns to the current face', () => {
    expect(resolveCustomMeshFaceSwitch('face-a', 'face-a', 'face-b')).toEqual({
      accept: true,
      pendingFaceId: null,
    })
  })
...
describe('clampCustomMeshFacePosition', () => {
  test('keeps the complete wall item inside the face after snapping', () => {
    expect(
      clampCustomMeshFacePosition([1.9, 2.8, 0], { minU: -2, maxU: 2, minV: 0, maxV: 3 }, [1, 1]),
    ).toEqual([1.5, 2, 0])
  })
  ...
  test('rejects a face that is smaller than the item', () => {
    expect(
      clampCustomMeshFacePosition(
        [0, 0, 0],
        { minU: -0.25, maxU: 0.25, minV: 0, maxV: 0.5 },
        [1, 1],
      ),
    ).toBeNull()
  })
...
describe('clampCustomMeshFaceCenterPosition', () => {
