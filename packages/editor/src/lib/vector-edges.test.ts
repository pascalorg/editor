...
function sceneWithProxy() {
  ...
  // Bones' utility-pole pick proxy: a tall box that draws nothing
...
describe('vector edges skip what draws nothing', () => {
  test('a pick proxy (colorWrite false) has no lines', () => {
    ...
    for (let i = 1; i < world.length; i += 3) expect(Math.abs(world[i] as number)).toBeLessThanOrEqual(0.5)
  })
  ...
  test('hidden materials, see-through hit boxes and colour-masked colliders all count as undrawn', () => {
    ...
    expect(materialShows(new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 }))).toBe(false)
    expect(materialShows([new THREE.MeshBasicMaterial({ colorWrite: false }), new THREE.MeshBasicMaterial()])).toBe(true)
  })
