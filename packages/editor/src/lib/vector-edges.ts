...
function materialShows(material: THREE.Material | THREE.Material[]): boolean {
  const list = Array.isArray(material) ? material : [material]
  return list.some((m) => m && m.visible !== false && !(m.transparent && m.opacity <= 0.02))
}
...
export async function extractVisibleEdges(
  ...
  const pass = await bufferPass(renderer, scene, camera, width, height, vec4(positionView.z.negate(), 0, 0, 1))
  if (!pass) return null
  const { data: depth, topDown } = pass
  const normalPass = await bufferPass(renderer, scene, camera, width, height, vec4(normalView, 1))
  const normals = normalPass?.data ?? null

  // ── candidate edges, world space ─────────────────────────────────────
  const world: number[] = []
