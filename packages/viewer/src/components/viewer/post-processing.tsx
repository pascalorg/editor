...
const PostProcessingPasses = ({
  ...
  useEffect(() => {
    ...
    // `navigator.gpu`, no secure context, or the device couldn't be created), building the
    ...
    const hasWebGPUBackend =
      backend?.isWebGPUBackend === true || backend?.constructor?.name === 'WebGPUBackend'
    const hasWebGPU =
      typeof navigator !== 'undefined' && 'gpu' in navigator && hasWebGPUBackend
    if (!hasWebGPU) {
      console.warn(
