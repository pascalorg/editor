...
function createSteelBeamOnCurrentLevel(start: WallPlanPoint, end: WallPlanPoint) {
  const currentLevelId = useViewer.getState().selection.levelId
  const { createNode, nodes } = useScene.getState()
  const dx = end[0] - start[0]
  const dz = end[1] - start[1]
  if (!(currentLevelId && dx * dx + dz * dz >= 0.01 * 0.01)) return null
  const count = Object.values(nodes).filter((node) => node.type === 'steel-beam').length
  const tray = SteelBeamNode.parse({
    name: `Steel Beam ${count + 1}`,
    start,
    end,
  })
...
export default function SteelBeamTool() {
  ...
  useEffect(() => {
    ...
    const onGridMove = (event: GridEvent) => {
      ...
      if (buildingState.current === 1) {
        ...
        const length = Math.hypot(
          snapped[0] - startingPoint.current.x,
          snapped[1] - startingPoint.current.z,
        )
        setLengthLabel(
          length >= 0.01
            ? {
    ...
    const onGridClick = (event: GridEvent) => {
      ...
      if (buildingState.current === 0) {
        ...
        const created = createSteelBeamOnCurrentLevel(
