...
export const CurvePipeTool: React.FC<{ node: PipeNode }> = ({ node }) => {
  ...
  useEffect(() => {
    ...
    const applyPreview = (curveOffset: number) => {
      if (previewOffsetRef.current === curveOffset) return
      previewOffsetRef.current = curveOffset

      const nextNode = { ...node, curveOffset }
      const handlePoint = getWallMidpointHandlePoint(nextNode)
      setCursorLocalPos([handlePoint.x, node.elevation, handlePoint.y])
      useScene.getState().updateNode(nodeId, { curveOffset })
      useScene.getState().markDirty(nodeId as AnyNodeId)
    }

    const restoreOriginal = () => {
      if (previewOffsetRef.current === originalCurveOffset) return
  ...
  if (isPipeNearlyVertical(node)) return null
