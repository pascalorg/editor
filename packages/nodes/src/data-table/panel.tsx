...
import { type AnyNode, type DataTableNode, useLiveData, useScene } from '@pascal-app/core'
import {
  ActionButton,
  ActionGroup,
  ColorAlphaField,
...
export default function DataTablePanel() {
  ...
  const handleAddRow = useCallback(() => {
    if (!node) return
    const nextOption =
      paths.find((option) => !node.rows.some((row) => row.dataKey === option.path)) ?? paths[0]
    handleUpdate({
      rows: [
        ...node.rows,
        {
          label: nextOption?.label ?? 'Metric',
          dataKey: nextOption?.path ?? 'machine.temperature',
        },
      ].slice(0, 8),
    })
  ...
  const rowDataKeys = node.rows.map((row) => row.dataKey)
  const pathOptions = [
    ...rowDataKeys
      ...
      <PanelSection title="数据行">
        <div className="space-y-2">
          {node.rows.map((row, index) => (
            <div className="space-y-1.5 rounded-lg border border-border/40 p-2" key={index}>
              <div className="flex items-center gap-2">
                <input
                  className="h-9 min-w-0 flex-1 rounded-lg border border-border/50 bg-[#2C2C2E] px-2 text-foreground text-xs"
