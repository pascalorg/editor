...
test.each(['duct-segment', 'pipe-segment'] as const)(
  '%s releases its wall plane after an outward commit and retains it along the wall',
  async (toolName) => {
    ...
    if (!previousWindow) Object.defineProperty(globalThis, 'window', { value: new EventTarget(), configurable: true })
    ...
    function Host() {
      const { gl } = useThree()
      gl.domElement.getBoundingClientRect = () => ({ left: 0, top: 0, width: 100, height: 100 }) as DOMRect
    ...
    const wallEvent = (x: number): GridEvent => ({
      ...
      localPosition: [x, 1, 0],
      surfaceNormal: [0, 0, 1],
      surfaceHit: { kind: 'wall', hostId: wall.id, levelId: 'level_test', face: 'side', side: 'front' },
    ...
    try {
      ...
      await act(async () => { emitter.emit('grid:click', wallEvent(1)) })
      await act(async () => { emitter.emit('grid:click', wallEvent(2)) })
      expect(commits).toEqual([[2, 1, 0.1]])
      await act(async () => {
        emitter.emit('grid:move', {
          position: [3, 0, 4], localPosition: [3, 0, 4],
          localRay: { origin: [3, 2, 2.1], direction: [0, -0.5, -1] },
          nativeEvent: {} as GridEvent['nativeEvent'],
        })
      })
      expect(tool?.cursor?.[2]).toBeCloseTo(0.1)
      await act(async () => { tool?.onDirectionSelect([0, 0, 1]) })
      const outward: GridEvent = {
        position: [2, 0, 2.1], localPosition: [2, 0, 2.1],
        localRay: { origin: [2, 3, 2.1], direction: [0, -1, 0] },
        nativeEvent: {} as GridEvent['nativeEvent'],
      }
      acceptCommit = false
      await act(async () => { emitter.emit('grid:click', outward) })
