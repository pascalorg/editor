...
export const MoveCableTrayEndpointTool: React.FC<{ target: MovingCableTrayEndpoint }> = ({
  target,
}) => {
  const cableTrayId = target.cableTray.id
  const endpoint = target.endpoint
  const initialPoint: [number, number] =
    endpoint === 'start'
      ? [target.cableTray.start[0], target.cableTray.start[1]]
      : [target.cableTray.end[0], target.cableTray.end[1]]
  const [cursorLocalPos, setCursorLocalPos] = useState<[number, number, number]>([
    initialPoint[0],
    target.cableTray.elevation + 0.1,
    initialPoint[1],
  ])
  const originalStartRef = useRef<[number, number]>([...target.cableTray.start] as [number, number])
  const originalEndRef = useRef<[number, number]>([...target.cableTray.end] as [number, number])
  ...
  useEffect(() => {
    ...
    const walls = getCurrentLevelWalls(target.cableTray.parentId ?? null)
    ...
    const applyPreview = (start: [number, number], end: [number, number]) => {
      ...
      setCursorLocalPos([movingPoint[0], target.cableTray.elevation + 0.1, movingPoint[1]])
      useScene.getState().updateNode(cableTrayId, { start, end })
      useScene.getState().markDirty(cableTrayId as AnyNodeId)
    }

    const restoreOriginal = () => {
      useScene.getState().updateNode(cableTrayId, { start: originalStart, end: originalEnd })
      useScene.getState().markDirty(cableTrayId as AnyNodeId)
    }
    ...
    const commitAtCursor = () => {
      ...
      useScene.getState().updateNode(cableTrayId, { start: preview.start, end: preview.end })
      useScene.getState().markDirty(cableTrayId as AnyNodeId)
      pauseSceneHistory(useScene)
      committed = true
      exitMoveMode(true)
  ...
  }, [endpoint, exitMoveMode, cableTrayId, target.cableTray])
...
export default MoveCableTrayEndpointTool
