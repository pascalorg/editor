...
export function cornerLinkedSourceModuleForRun(
  ...
  return (
    cabinetModulesForRun(run, nodes).find((module) => cornerSourceLink(module.metadata) != null) ??
    null
  )
}
...
function computeCornerRunLayout({
  ...
  if (connectedWidth < MIN_CORNER_CONNECTED_WIDTH - WALL_CLEARANCE_EPSILON) return null
...
function resolveCornerAdditionLayout({
  ...
  if (directLayout) return { sourceModule: module, layout: directLayout }

  for (
    let sourceWidth = module.width - CORNER_WIDTH_SEARCH_STEP;
    sourceWidth >= MIN_CORNER_CONNECTED_WIDTH - WALL_CLEARANCE_EPSILON;
    sourceWidth -= CORNER_WIDTH_SEARCH_STEP
  ) {
    const candidateModule = adjustedCornerSourceModule(module, side, Number(sourceWidth.toFixed(4)))
    ...
    if (layout) return { sourceModule: candidateModule, layout }
  }
...
function syncDerivedCornerRun({
  ...
  const anchorPosition =
    role === 'base-leg'
      ...
      : role === 'wall-leg'
        ...
        : modules.length === 1 && modules[0]?.name === 'Wall Bridge Filler'
          ? layout.bridgeFillerRunPosition
  ...
  const runWorldPosition = runLocalToPlan({ position: anchorPosition, rotation }, [
...
export function syncCornerRunsFromSourceModule({
  ...
  for (const runId of link.linkedRunIds) {
    ...
    if (!linkedRun || linkedRun.type !== 'cabinet') continue
...
export function addCornerRun({
  ...
  if (run.runTier !== 'base' || resolveCabinetType(module, run) !== 'base') return null
  ...
  const isEndModule =
    side === 'left'
      ? Math.abs(moduleMinX(module) - extent.minX) <= CABINET_EDGE_EPSILON
      : Math.abs(moduleMaxX(module) - extent.maxX) <= CABINET_EDGE_EPSILON
  ...
  const { layout, sourceModule: resolvedSourceModule } = resolved
  let sourceModule = module
  let sourceRun = run
  if (
    resolvedSourceModule.width < module.width - CABINET_EDGE_EPSILON ||
    layout.connectedWidth < module.width - CABINET_EDGE_EPSILON
  ) {
    ...
    sceneApi.update(module.id as AnyNodeId, sourcePatch as Partial<AnyNode>)
    const existingWallTop = wallChildOf(module, sceneApi.nodes())
    if (existingWallTop) {
      sceneApi.update(
    ...
    sourceModule = sceneApi.get<CabinetModuleNode>(module.id as AnyNodeId) ?? module
    sourceRun = sceneApi.get<CabinetNode>(run.id as AnyNodeId) ?? run
  }
  ...
  const existingWallTop = wallChildOf(sourceModule, sceneApi.nodes())
  ...
  const wallLocalRotation = worldToCabinetLocalRotation(sourceRun, sceneApi.nodes(), legRotation)
  const bridgeLocalRotation = worldToCabinetLocalRotation(
    sourceRun,
    sceneApi.nodes(),
    runWorld.rotation,
  )
  ...
  const bridgeModules = existingWallTop
    ? side === 'right'
      ? [
          {
  ...
  sceneApi.update(baseLeg.runId, {
    metadata: {
      ...cabinetMetadataRecord(sceneApi.get<CabinetNode>(baseLeg.runId)?.metadata ?? null),
  ...
  for (const segment of uncoveredWallRunSegments({
    depth: CABINET_WALL_DEPTH,
    modulePatches: bridgeModules,
    parentId: sourceRun.id as AnyNodeId,
    position: existingWallTop ? bridgeFillerRunPosition : bridgeRunPosition,
    rotation: runWorld.rotation,
    sceneApi,
  })) {
    const bridgeSegmentLocalPosition = worldToCabinetLocalPosition(
      sourceRun,
      sceneApi.nodes(),
      segment.position,
    )
    const bridgeRun = upsertCabinetRunWithModules({
      depth: CABINET_WALL_DEPTH,
      modulePatches: segment.modulePatches,
      name: 'Corner Wall Bridge',
      parentId: sourceRun.id as AnyNodeId,
      position: bridgeSegmentLocalPosition,
    ...
    sceneApi.update(bridgeRun.runId, {
      metadata: {
        ...cabinetMetadataRecord(sceneApi.get<CabinetNode>(bridgeRun.runId)?.metadata ?? null),
        cabinetCornerDerivedRun: {
          role: 'bridge',
          side,
          sourceModuleId: sourceModule.id as AnyNodeId,
  ...
  for (const segment of uncoveredWallRunSegments({
    depth: CABINET_WALL_DEPTH,
    modulePatches: wallLegModules,
    parentId: sourceRun.id as AnyNodeId,
    position: wallRunPosition,
    rotation: legRotation,
    sceneApi,
  })) {
    const wallSegmentLocalPosition = worldToCabinetLocalPosition(
      sourceRun,
      sceneApi.nodes(),
      segment.position,
    )
    const wallLeg = upsertCabinetRunWithModules({
      depth: CABINET_WALL_DEPTH,
      modulePatches: segment.modulePatches,
      name: 'Corner Wall Run',
      parentId: sourceRun.id as AnyNodeId,
      position: wallSegmentLocalPosition,
      rotation: wallLocalRotation,
      runTier: 'wall',
      sceneApi,
      sourceRun,
    })
    linkedRunIds.push(wallLeg.runId)
    sceneApi.update(wallLeg.runId, {
      metadata: {
        ...cabinetMetadataRecord(sceneApi.get<CabinetNode>(wallLeg.runId)?.metadata ?? null),
        cabinetCornerDerivedRun: {
          role: 'wall-leg',
          side,
          sourceModuleId: sourceModule.id as AnyNodeId,
