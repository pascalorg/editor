...
function getPascalTruckAssetSrc() {
  if (typeof window === 'undefined') {
    return PASCAL_TRUCK_ASSET.src
  }

  return new URL(PASCAL_TRUCK_ASSET.src, window.location.origin).toString()
}

function getPascalTruckNodeAsset(): ItemNode['asset'] {
  return {
    ...PASCAL_TRUCK_NODE_ASSET,
    src: getPascalTruckAssetSrc(),
  }
}

export function getPascalTruckLocalAsset(): ItemNode['asset'] {
  return getPascalTruckNodeAsset()
}
...
function computePascalTruckSeedTransform(sceneGraph: SceneGraph, levelId: string | null) {
  ...
  const fallbackTargetCenter = getLevelGeometryCenter(sceneGraph, levelId)
  ...
  const targetCenter = fallbackTargetCenter ?? getPolygonCenter(sitePolygon)
...
export function buildPascalTruckNodeForScene(
  ...
  const truckAsset = getPascalTruckNodeAsset()
