...
function ensureItemDoorLeafPivot(doorRoot: Object3D) {
  const existingLeafPivot = doorRoot.getObjectByName('door-leaf-pivot')
  if (existingLeafPivot) {
    return existingLeafPivot
  }

  const movableChildren = doorRoot.children.filter((child) => child.name !== 'door-leaf-pivot')
  ...
  leafPivot.userData.pascalNavigationRuntime = true

  const leafGroup = new Group()
  leafGroup.name = 'door-leaf-group'
  leafGroup.userData.pascalNavigationRuntime = true
...
function ensureItemDoorAnimationState(doorId: string) {
  ...
  const leafPivot = doorRoot ? ensureItemDoorLeafPivot(doorRoot) : undefined
