...
| `validate_formwork` | Whether the formwork in scope can be erected — 18 invariants over the solved layout, pressure envelope, catalog system, drilled tie grid and corner hardware. | `{ levelId?, elementIds? }` | `{ errorCount, warningCount, findings[], summary[], shutteredIds[], notChecked[] }` |
