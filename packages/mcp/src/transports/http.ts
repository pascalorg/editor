...
export type HttpTransportOptions = {
   ...
   */
  host?: string
  /** Bearer token for HTTP MCP calls. Defaults to PASCAL_API_TOKEN. */
  authToken?: string
...
export async function connectHttp(
  ...
  const authToken = options.authToken ?? process.env.PASCAL_API_TOKEN
  if (!(isLoopbackHost(host) || authToken)) {
    throw new Error('HTTP transport on a non-loopback host requires PASCAL_API_TOKEN or authToken')
  }
