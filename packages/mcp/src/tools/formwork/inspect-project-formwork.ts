...
export const inspectProjectFormworkOutput = {
    ...
    .array(
      z.object({
        ...
        moves: z.array(
          z.object({
            /**
             * The handle `apply_pour_move` takes, and the only way to obtain one.
             *
             * It carries the shift as well as the pour, unlike a finding's key, because the
             * figure is the decision: a key read here names the move this programme offers, and
             * the same pour offers a different shift the moment another pour is dated.
             */
            key: z.string(),
            pourId: z.string(),
            assemblyIds: z.array(z.string()),
  ...
  /**
   * Every pick on the job, heaviest first — what the hook lifts at a time.
   *
   * The one block here that comes off the geometry rather than off the bill or the programme,
   * and the only one a caller can confuse with `totalWeightKg`: that is what passes through
   * the job and a pick is one hook load. `heaviestPickKg` is deliberately the only weight
   * figure at the block's top level — there is no sum of picks, because picks happen one at a
   * time and a total of them is a load nothing ever lifts.
   */
  lifting: z
    .object({
      picks: z.number(),
      /** The figure a crane is chosen against. Null where no pick in scope has a weight. */
      heaviestPickKg: z.number().nullable(),
      unweighedPicks: z.number(),
      overTheChartPicks: z.number(),
      positionDependentPicks: z.number(),
      overHookHeightPicks: z.number(),
      /**
       * The chart these were read against. Null where none is recorded, which means no pick
       * below has been checked against a lift at all.
       */
      crane: z
        .object({
          worstCapacityKg: z.number(),
          bestCapacityKg: z.number(),
          reachFromM: z.number(),
          reachToM: z.number(),
          hookHeightMm: z.number().nullable(),
        })
        .nullable(),
      items: z.array(
        z.object({
          ...
          heightMm: z.number(),
          panels: z.number(),
          pickWeightKg: z.number().nullable(),
          /**
           * What the chart says: `lifts`, `position` or `over-chart`. Null where there is no
           * chart, which is not a pick that passed.
           */
          verdict: z.string().nullable(),
          /** The furthest published radius that still takes it. Only set on `position`. */
          liftsInsideM: z.number().optional(),
          slingsWantMm: z.number().nullable(),
          overHookHeight: z.boolean().optional(),
          overAStatedLimit: z.boolean().optional(),
        }),
      ),
      excludes: z.array(z.string()),
    })
    .optional(),
  /**
   * Why there is no `lifting` block at all.
   *
   * The absence a caller is most likely to read as a fault, because it is the one block here
   * whose input is geometry rather than a setting: no gang means a conventional shutter struck
   * panel by panel, which is a real way to build and not a missing input.
   */
  noLiftingBecause: z.string().optional(),
  /**
   * The deliveries and the hook time — the two costs `cost.excludes` has named since there
   * was a cost.
   *
   * The only block here derived from two others: the loads from `totalWeightKg` and the hours
   * from `lifting.picks`. Both halves are independent, so a project that has stated a payload
   * and no cycle time gets loads and no crane hours, which is a real state rather than a gap.
   */
  logistics: z
    .object({
      currency: z.string().nullable(),
      /** The fewest trips a job of this weight takes, out and back. */
      loads: z.number().nullable(),
      loadsOut: z.number().nullable(),
      loadsBack: z.number().nullable(),
      lorryPayloadKg: z.number().nullable(),
      weightTheLoadsCameFromKg: z.number().nullable(),
      transportCost: z.number().nullable(),
      picks: z.number().nullable(),
      /** This formwork's cycles alone, and a charge only on a crane hired by the hour. */
      hookHours: z.number().nullable(),
      craneCost: z.number().nullable(),
    ...
    .optional(),
  /** Why there is no `logistics` block at all. */
  noLogisticsBecause: z.string().optional(),
  /**
   * The sheets the cut ply comes out of, where the project stated what the yard buys.
   *
   * The one block here a caller must not fold into any total, and the only one whose figures
   * are a second count of material already in `bom`: every board nested here is a ply line
   * above, priced as consumed and weighed into the tonnage. So `excludes` leads with that, and
   * `sheetsToBuy` is deliberately not beside a cost or a weight anywhere in this answer.
   */
  cutList: z
    .object({
      /** Sheets to buy, per stock id — the orderable line. */
      sheetsToBuy: z.array(z.object({ sheetId: z.string(), sheets: z.number() })),
      /** The same order with the yard's stated damage allowance on top, where it stated one. */
      sheetsToBuyWithHandlingAllowance: z
        .array(z.object({ sheetId: z.string(), sheets: z.number() }))
        .nullable(),
      boardsNested: z.number(),
      boardAreaM2: z.number(),
      /** Sheet area the boards did not take, over the sheet area bought. Geometric. */
      cuttingWastePercent: z.number(),
      sawKerfMm: z.number(),
      /** Offcut the stated policy keeps, m² — reported and never netted off the sheets. */
      offcutWorthRackingM2: z.number(),
      /** Boards no stated sheet holds. Named rather than divided, so the count is short. */
    ...
    .optional(),
  /** Why there is no `cutList` block at all. */
  noCutListBecause: z.string().optional(),
      ...
      return textResult({
        ...
        ...(solution.commitments
          ...
          : solution.schedule
            ...
            : {}),
        // The lifting schedule, off the same layout the panels and the bill came from. Absent
        // where nothing in scope is ganged, which is a conventional shutter rather than a
        // missing input — `noLiftingBecause` is what stops that reading as a fault.
        ...(solution.lifts
          ? {
              lifting: {
                ...
                items: solution.lifts.picks.map((pick) => ({
                  ...
                  // Null rather than 0 where a piece in the gang carries no stated weight. A 0
                  // sails under any capacity check, which is the one number here a caller would
                  // repeat to a user as a pick a crane takes.
        ...
        // Off the weight and the picks above rather than a second sweep of either, so a caller
        // can check the division. Absent where the project has recorded neither a payload nor
        // a cycle time, which is the commonest case and is why `cost.excludes` has named these
        // two from the day there was a cost.
        ...(solution.logistics
          ? {
              logistics: {
        ...
        // Last, after the bill it must not be added to. Two different absences below and both are
        // real states: a steel-panel job has no ply to cut, and a job that cuts ply has no cut
        // list until somebody says which sheet the yard buys — the second is a missing input and
        // the first is not, so they get different sentences rather than one hedged one.
        ...(solution.cutList
          ? {
              cutList: {
                sheetsToBuy: solution.cutList.list.order,
                sheetsToBuyWithHandlingAllowance: solution.cutList.list.orderWithAllowance ?? null,
                boardsNested: solution.cutList.boardCount,
                boardAreaM2: solution.cutList.boardAreaM2,
                cuttingWastePercent: Math.round(solution.cutList.list.cuttingWasteFraction * 100),
                sawKerfMm: solution.cutList.list.kerfMm,
                offcutWorthRackingM2: round(solution.cutList.list.retainableAreaMm2 / 1_000_000),
                boardsLargerThanEverySheet: solution.cutList.list.oversize.map((piece) => ({
                  mark: piece.mark,
                ...
                excludes: [
                  'the bill above, which already lists every one of these boards as cut ply — the sheets are that same material counted a second way, so adding them to any total here buys the job’s plywood twice',
                  'the weight, the owned/hired split and the cost, none of which this touches',
                  'any board larger than every stated sheet, which is named rather than divided',
                  'whether the yard racks the offcut this says it could, so the retained area is reported and never netted off the sheets',
                ],
              },
            }
          : {
              noCutListBecause: solution.bom.some((line) => line.kind === 'ply-piece')
                ? 'This job cuts ply and the project has not recorded which sheet the yard buys, so there is no cut list. Never take the sheet from a sheathing grade and never pick one: a grade carries the pressures it takes and no width or length at all, and a merchant selling 1220 × 2440 and one selling 1250 × 2500 give the same wall different sheet counts — which sheet is bought is a commercial fact about this job. Ask the user which sheet the yard buys and record it with set_formwork_settings sheets.'
                : 'Nothing in scope is cut out of sheet material — every part in the bill is a standard product — so there is no cutting to do here. Not a missing input, and a sheet count of zero would read as a job whose ply is free rather than as a job with no ply in it.',
            }),
