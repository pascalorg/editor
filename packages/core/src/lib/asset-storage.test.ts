...
describe('sweepOrphanAssets', () => {
  ...
  test('returns zero when every asset is still referenced', async () => {
    const url = await saveAsset(file('still-used'))
    // Prior tests leave unrelated orphans in the shared fake IDB; only the
    // asset saved in this case is expected to survive.
    await sweepOrphanAssets([url])
    expect(await sweepOrphanAssets([url])).toBe(0)
    expect(await loadAssetUrl(url)).not.toBeNull()
  })
})
