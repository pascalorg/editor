...
export function buildMergedWallAttachmentUpdates(
  ...
  // Children are placed by arc distance along the centerline, so a merged curved
  // wall keeps them on the curve instead of the chord.
  const mergedGeometry = { start: mergedStart, end: mergedEnd, curveOffset: primary.curveOffset }
  const mergedLength = Math.max(getWallCurveLength(mergedGeometry), 1e-6)
  const chordLength = Math.max(
    Math.hypot(mergedEnd[0] - mergedStart[0], mergedEnd[1] - mergedStart[1]),
    1e-6,
  )
  const tangentX = (mergedEnd[0] - mergedStart[0]) / chordLength
  const tangentZ = (mergedEnd[1] - mergedStart[1]) / chordLength
  ...
  for (const childId of wallChildren) {
    ...
    const nextLocalX = Math.max(
      0,
      Math.min(mergedLength, projectPointToWallCenterline(mergedGeometry, sourcePoint).distanceAlong),
    )
