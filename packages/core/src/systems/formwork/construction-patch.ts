...
/** Declared on `SlabNode` alone, so refused on the other two kinds rather than dropped. */
const SLAB_ONLY_CONSTRUCTION_INPUT = {
  ...
  soffitHeightAboveSupport: z
    ...
    .optional()
    .describe(
      'slabs only: meters from the floor the props stand on to the soffit. This is the prop length, so it drives the prop capacity and the whole falsework design',
    ),
} as const
...
export function applyConstructionPatch(
  ...
  for (const field of stated) {
    ...
    changed.push(value === null ? `${field} unstated` : `${field} ${String(value)}`)
  }
...
export function castableElementSummary(
  node: WallNode | ColumnNode | SlabNode,
