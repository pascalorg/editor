...
This list describes the 10 real-world IFC (Industry Foundation Classes) example files that the converter app offers for testing. Four of them (01, 04, 05 and 10) are checked in here and served from the app's `public/` folder; the other six (02, 03 and 06–09) are fetched at runtime from a public Supabase Storage bucket (`lib/test-files.ts`). Most are BIM models exported from authoring tools such as Revit and ArchiCAD; 04 is generated by a program (IfcOpenShell's IfcOpenHouse example). They were gathered from public repositories. Being publicly downloadable does not mean they may be reused: see [Licence status](#licence-status) before using any of them.
...
- **Currently served**: the bucket's `02-schependomlaan.ifc` is byte-identical to `06-sample-castle.ifc`, so the app converts the castle when 02 is picked (see [Licence status](#licence-status))
...
| 02-schependomlaan.ifc | Supabase bucket | Listed as "10 Appartementen Schependomlaan", via xeokit-sdk / buildingSMART sample files. **The bucket object is not that model: it is byte-identical to 06-sample-castle.ifc** (SHA-256 `2c3565ca…bec7`, same size and ETag), so choosing 02 currently converts the castle. | Not verified for Schependomlaan; what is served is the castle, with 06's status. |
