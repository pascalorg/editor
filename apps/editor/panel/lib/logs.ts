...
export async function recentActors(
  minutes = 20,
): Promise<Array<{ actor: string; lastAction: string; at: string }>> {
  const rows = await query<
    RowDataPacket & { actor_label: string; message: string; created_at: Date }
  >(
