...
export async function listUsers(opts: UserQuery = {}): Promise<UserListResult> {
  ...
  const filtered = rows.filter((row) => {
    ...
    const haystack =
      `${row.full_name} ${row.email} ${row.username} ${row.global_role}`.toLocaleLowerCase(
        opts.lang === 'tr' ? 'tr' : 'en',
      )
  ...
  const users = await Promise.all(
    slice.map(async (row) => {
      ...
      return row.status === 'invited'
        ? { ...user, invitation: await invitationForUser(row.id) }
        : user
    }),
  )
  ...
  return transaction(async (cx) => {
    await cx.execute(
      ...
      [
        ...
        input.fullName,
        input.org,
        clampRole(input.org, input.role),
      ],
    )
    ...
    if (userId === undefined) throw new Error('user row vanished after insert')
...
export async function setAssignments(
  ...
  await transaction(async (cx) => {
    for (const [siteName, role] of Object.entries(siteRoles)) {
      ...
      if (role === null) {
        await cx.execute('DELETE FROM assignments WHERE user_id = ? AND site_id = ?', [
          userId,
          siteId,
        ])
        continue
