...
const SYSTEM: Record<string, Permission[]> = {
  ...
  Editor: ['edit_projects', 'create_projects', 'delete_projects', 'access_settings', 'view_projects'],
...
async function loadRoles(): Promise<Map<string, RoleDefinition>> {
  ...
  try {
    const rows = await query<RowDataPacket & { name: string; permissions: unknown; is_system: number }>(
