...
import { query, queryOne, exec, type RowDataPacket } from '../db';
...
export function packIp(raw: string | null): Buffer | null {
  ...
  if (kind === 6) {
    ...
    groups.forEach((g, i) => buf.writeUInt16BE(parseInt(g, 16) & 0xffff, i * 2));
    return buf;
  }
  return null;
}
...
export async function createSession(opts: {
  ...
  await exec(
    `INSERT INTO sessions (id, user_id, device, ip, trusted_until, keep_signed_in, mfa_pending,
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), ?)`,
    [id, opts.userId, device, ip, opts.trustedUntil ?? null, keepSignedIn ? 1 : 0, opts.mfaPending ? 1 : 0, expiresAt],
  );
...
async function enforceConcurrencyLimit(userId: number, keepId: Buffer, limit: number): Promise<void> {
  if (limit <= 0) return;
...
export async function getSession(opts: { touch?: boolean } = {}): Promise<ActiveSession | null> {
  ...
  if (opts.touch !== false) {
    ...
    const nextExpiry = row.keep_signed_in === 1
      ? row.expires_at
      : new Date(Date.now() + settings.sessionMinutes * 60 * 1000);
    await exec('UPDATE sessions SET last_activity_at = NOW(), expires_at = ? WHERE id = ?', [nextExpiry, id]);
  ...
  const state: AuthState =
    row.mfa_pending === 1 ? 'mfaRequired'
    : row.must_change_password === 1 ? 'firstSignIn'
    : 'signedIn';
...
export async function revokeAllSessions(userId: number, except?: Buffer | null): Promise<number> {
  const res = except
    ...
    : await exec('UPDATE sessions SET revoked_at = NOW() WHERE user_id = ? AND revoked_at IS NULL', [userId]);
 ...
 */
async function effectivePermissions(globalRole: string, siteRoles: Record<string, Role>): Promise<Permission[]> {
