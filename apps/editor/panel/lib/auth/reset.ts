...
export async function issueReset(userId: number, ip: string | null): Promise<IssuedReset> {
  ...
  await exec('UPDATE password_resets SET used_at = NOW() WHERE user_id = ? AND used_at IS NULL', [
    userId,
  ])
  await exec(
...
export async function markResetUsed(resetId: number): Promise<void> {
  await exec('UPDATE password_resets SET used_at = NOW() WHERE id = ? AND used_at IS NULL', [
    resetId,
  ])
}
