...
const DEV_USERS = [
  {
    name: 'Resul Övür',
    username: 'r.ovur',
    email: 'resul.ovur@netlog.com.tr',
    password: 'ro12345',
  },
  {
    name: 'Cengiz Tuna',
...
async function main() {
  ...
  const [adminRows] = await cx.execute<mysql.RowDataPacket[]>(
  ...
  console.log(
    `  ✓ admin (${adminUsername} / ${adminEmail}) — must change password on first sign-in`,
  )
