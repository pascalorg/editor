...
export function MfaSetupScreen() {
  ...
  const verify = useCallback(
    async (joined?: string) => {
      ...
      const res = await call<MfaVerifyResponse>('/api/mfa/verify', {
        body: { code: value, trustDevice: false },
      })
      setBusy(false)
  ...
  const title =
    step === 'scan' ? t.muTitleScan : step === 'verify' ? t.muTitleVerify : t.muTitleCodes
          ...
          <>
            <div className="flex items-stretch gap-[14px]">
              ...
              <div className="flex min-w-0 flex-1 flex-col justify-center gap-2">
                ...
                <span className="break-all font-mono text-xs tracking-[0.04em] text-fg">
                  {manualKey || '····'}
                </span>
                <button
                  type="button"
