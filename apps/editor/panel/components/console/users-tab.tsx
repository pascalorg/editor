...
import type { BulkUsersResponse, PendingRequestsResponse, UsersListResponse } from '@panel/lib/api-contract';
import { format, resolveApiMessage, type Dictionary } from '@panel/lib/i18n';
    ...
    <section className="flex min-w-0 flex-col gap-[14px]" style={{ animation: 'dtFade 0.2s ease' }}>
      ...
      {bulk ? (
        ...
        >
          ...
          <p className="m-0 text-[11.5px] leading-[1.5] text-muted-fg text-pretty">{t.bulkSkipNote}</p>
          ...
          <div className="flex flex-col gap-[9px]">
            <Button
              ...
              disabled={bulkBusy || (bulk === 'delete' && confirmCount.trim() !== String(selectedOnPage.length))}
...
function UserCard({
  ...
  return (
    ...
    <div className={cn('flex items-start gap-2 border-b border-border-soft px-3 py-3 last:border-b-0', checked && 'bg-hover')}>
      {selectable ? (
        <input
          type="checkbox"
          checked={checked}
...
function reasonLabel(t: Dictionary, reason: BulkUsersResponse['skipped'][number]['reason']): string {
  switch (reason) {
    case 'primaryAdmin':
