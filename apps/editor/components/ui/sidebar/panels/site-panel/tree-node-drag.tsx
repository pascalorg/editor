...
function getTransform(node: AnyNode): Transform {
  const pos = 'position' in node && Array.isArray(node.position)
...
export function TreeNodeDragProvider({ children }: { children: ReactNode }) {
  ...
  const startDrag = useCallback(
    (nodeId: string, nodeType: string, sourceParentId: string, label: string, x: number, y: number) => {
...
function FloatingPreview({ drag }: { drag: NonNullable<DragState> }) {
  return createPortal(
    <div
      className="pointer-events-none fixed z-[200] flex items-center gap-1.5 rounded-lg border border-accent bg-background/95 px-2.5 py-1.5 text-xs font-medium text-foreground shadow-xl backdrop-blur-sm"
      style={{
