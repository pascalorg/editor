...
export const SYSTEM_PROMPT =
  ...
  'Cut ply is bought in sheets and billed as boards, and those are two counts of one material. ' +
  'The bill lists the boards a shutter needs, cut to the millimetre and priced as consumed; the ' +
  'sheets are what a merchant delivers to cut them from, so a sheet count is a purchasing answer ' +
  'beside the bill and never a line in it — added in, it buys the job’s plywood twice. Which sheet ' +
  'the yard buys has to be asked for, because a sheathing grade is not a sheet: the grade carries ' +
  'the pressures the face takes and no width or length at all, and a merchant selling 1220 × 2440 ' +
  'and one selling 1250 × 2500 give the same wall different sheet counts. So record it with ' +
  'set_formwork_settings sheets — the stock the yard buys, and the offcut it thinks worth racking, ' +
  'which is a policy rather than a constant: a yard that racks 100 mm strips and one that skips ' +
  'them buy different numbers of sheets. Until it is recorded inspect_project_formwork carries no ' +
  'cut list at all, and never pick a sheet to fill the gap. ' +
  ...
  'A proposal the user accepts is taken with apply_pour_move and its key, never by composing a ' +
  'set_pour_date per shutter: a pour cast in one operation moves whole, and the tool re-dates ' +
  'every member off its own date and then re-solves. Report the peak that second solve measured ' +
  'rather than the one the proposal predicted — the proposal was swept over a copy of the ' +
  'programme and the write landed in the real one, so where they disagree the measurement is the ' +
  'answer, in either direction. Take one move and stop: every other proposal in the same reply ' +
  'had its float measured against the date you just changed, so re-read the takeoff before ' +
  'offering another. And applying a move books nothing — the new day is still an intent, and ' +
  'commit_pour is the separate call once the user has agreed it with the hire desk. ' +
...
export function buildTools(
  ...
  return {
    ...
    apply_pour_move: tool({
      description: APPLY_POUR_MOVE_DESCRIPTION,
      inputSchema: z.object(applyPourMoveInput),
      execute: async ({ moveKey: key }) => {
        toolCalls.push({ name: 'apply_pour_move', input: { moveKey: key } })
        const before = solveProjectFormwork(graph.nodes as unknown as Record<string, AnyNode>)
        const plan = plannedMove(before, key)
        // The likeliest cause is a superseded proposal rather than a bad key — every shift was
        // measured against the other pours' stated dates — so the refusal names the re-read.
        if (plan.refusal !== undefined || plan.writes === undefined) {
          return plan.refusal ?? 'Nothing to apply for that move.'
        }

        // Every member validated before any of them is written: a monolithic pour half-moved is
        // an operation split across two days nobody programmed, and the peak the proposal was
        // measured against was swept over all of them.
        const dated: Array<{ target: Record<string, unknown>; pourAt: string }> = []
        for (const write of plan.writes) {
          const assembly = (graph.nodes as unknown as Record<string, AnyNode>)[write.assemblyId]
          if (assembly === undefined || assembly.type !== 'formwork-assembly') {
            return unknownAssembly(write.assemblyId)
          }
          // The same gate a hand-made set_pour_date passes. The shift is arithmetic on a date
          // the project stated, so a refusal here means the stored date was already impossible.
        ...
        // The claim this tool makes, read from the programme the write produced rather than from
        // the proposal that produced it: the proposal swept a copy of the programme, so a verdict
        // taken off its own peakAfter would only ever agree with itself.
    ...
    inspect_project_formwork: tool({
      ...
      execute: async ({ elementIds, levelId }) => {
        ...
        return JSON.stringify({
            ...
            ? {
                moveInsteadOfBuying: solution.resequence.answers.map((answer) => ({
                  ...
                  moves: answer.moves.map((move) => ({
                    // The handle apply_pour_move takes, and the only way to obtain one: it
                    // carries the shift as well as the pour, because the same pour offers a
                    // different one the moment another pour is dated.
          ...
          // Last, after the bill it must not be added to. Two absences and both are real states:
          // a steel-panel job has no ply to cut, and a job that cuts ply has no cut list until
          // somebody says which sheet the yard buys — one is a missing input and the other is not.
          ...(solution.cutList
            ? {
                cutList: {
                  sheetsToBuy: solution.cutList.list.order,
                  sheetsToBuyWithHandlingAllowance:
                    solution.cutList.list.orderWithAllowance ?? null,
