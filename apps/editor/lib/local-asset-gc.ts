 ...
 * Returns null when any source cannot be enumerated — callers must skip GC
 * rather than sweep from a partial keep-set.
 */
export async function collectAllPersistedAssetUrls(
  ...
  // If listing fails we cannot prove the keep-set is complete.
  let scenesJson: { data?: { scenes?: unknown[] }; scenes?: unknown[] } | null = null
  try {
    const res = await fetch('/api/scenes?limit=100')
  ...
  for (const entry of list) {
    ...
    try {
      ...
      const graph =
        body.data?.graph ?? body.data?.scene?.graph ?? body.graph ?? null
...
export async function runLocalAssetGc(
  ...
  const keep = await collectAllPersistedAssetUrls(currentNodes)
  if (keep === null) return null
  return sweepLocalAssetsExcept(keep)
}
