...
import { updateUserSchema, type UserDetailResponse } from '@panel/lib/api-contract';
...
export const PATCH = handler(async (request: Request, ctx: { params: Promise<{ id: string }> }) => {
  ...
  if (before.isPrimaryAdmin && (parsed.data.status === 'Inactive' || parsed.data.role !== undefined)) {
    return fail('forbidden', 'err.primaryAdminProtected');
  }

  const internal = await queryOne<RowDataPacket & { id: number }>('SELECT id FROM users WHERE public_id = ?', [id]);
