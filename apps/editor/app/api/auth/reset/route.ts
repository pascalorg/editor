...
import { resetRequestSchema, type ResetRequestResponse } from '@panel/lib/api-contract';
...
export const POST = handler(async (request: Request) => {
  ...
  if (eligible) {
    ...
    const { token, expiresAt } = await issueReset(user.id, h.get('x-forwarded-for') ?? h.get('x-real-ip'));
