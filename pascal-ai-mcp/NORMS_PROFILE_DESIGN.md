...
状态：**已确认（2026-07-10）**，落码分批进行（见 §4）。落点全部在 pascal-ai-mcp（不动 packages/mcp）。
...
| J7 | 房间尺寸以 **帖** 对齐：**1帖=1.62㎡**（已拍板，通用值），面积取整到 0.25帖 | 网格对齐 | partitioner 面积/坐标量化 |
...
| DK | 卧室≤1：4.5帖；卧室≥2：**6帖** | 6–10帖 | 对应 dining（近似） |
| 独立キッチン | 3帖 ≈ 4.9㎡ | 3–4.5帖 | |
| 玄関 | 1帖 ≈ 1.6㎡ | 1.5–2帖 | |
| トイレ | 0.75帖 ≈ 1.2㎡（0.78×1.2m 起） | 1帖 | 方案 B 下与浴室同为 `bathroom` 类型，按 name 区分 |
| 浴室（UB） | 1216 规格 ≈ 1.92㎡ | 1616 规格 ≈ 2.56㎡ | **UB 规格绑定 areaBand（2026-07-10 修正）**：tiny/compact 默认 1216，standard 起 1616；按规格枚举，不按连续面积 |
| 洗面脱衣室 | **1帖 fatal / 2帖（1坪）comfortable（2026-07-10 修正）** | 1.5–2帖 | 家庭户型惯例 1坪 |
