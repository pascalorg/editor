...
状态：**已全部落地**（S0–S5，2026-07-10）。落点全部在 pascal-ai-mcp（不动 packages/mcp）。
...
>
> 姊妹文档：[NORMS_PROFILE_DESIGN.md](NORMS_PROFILE_DESIGN.md)（default / JP 规范档案与参数来源）。
...
buildLayoutPlan:
   ...
   ③ partitionLayout(intent, profile, strategy)（拓扑选择 + 参数 + 打分权重）
   ④ validateLayoutPlan(plan, targets, profile)（一致性硬校验 → plan_rejected 回路）
```
...
枚举值**只包含分区器当前真能执行的拓扑**。五个值已全部落地。判定按序：站点约束（l_shape、narrow_lot）优先于偏好拓扑（tanoji），单房间永远是 studio。
...
#### 水回り聚拢（2026-07-16，用户草图驱动）

两个机制，按户型结构分流；**modify 路径（带 previousPlan）都不启用**——老卫生间不该为了聚拢而搬家（M2 最小改动原则）。

- **湿区列（wet stack，band 拓扑通用，2026-07-16 由用户 03/04 草图驱动）**：原本雕进枢纽的卫生间改为叠在厨房柱上，两间共用一根管线柱——无走廊单排：卫生间占远端（z1）、厨房贴 z0 入户边，门都开向枢纽（03 草图「卧室｜客厅｜厨卫叠列」）；带走廊：z1 侧即走廊侧，卫生间贴走廊、厨房占外墙角，湿区列与枢纽相邻时卫生间门开向枢纽、隔着 dining 时退回开向走廊（04 草图「卧室带｜走廊（侧端入户）｜客厅+厨卫叠列」）。约束按两个格子分别查（min 边宽 + 长宽比），组合列不查。选型：湿区列与雕刻形是**并行候选**——新生成按偏好档胜出（同田の字先例，不混池比分）；modify 路径偏好档关闭，由位移惩罚自然保持旧图已有的形态（M2 最小改动）。
- **雕刻锚点（carve anchor，带走廊等其余情形）**：卫生间选角在**自己允许的角落集合内**按「离厨房最近」重排（厨房为独立柱时以柱中心为锚）——只改顺序、不改集合，「卫生间门朝走廊」规则不受影响。band / narrow_lot / l_shape 三个 carve 拓扑共用。局限：两个 carve 各占一角、之间必须留 ≥minDoorEdge 的宿主临墙段，是「同墙同侧」而非贴合；走廊型的严格贴合等水回りコア重构（TEMPLATES.md 校准清单 #4）。

#### narrow_lot 落地细则（S3）

- 单房间 brief 仍判 studio（singleRoomPlan 处理）；无公共枢纽（living/LDK/hallway 均缺）时分区器静默回退 standard 路径（corridor-hub 本身就是线性的）。
- 有 footprintHint 时地块尺寸固定为 footprint（短边为宽），与目标总面积冲突时以地块为准；无尺寸仅有"长条"关键词时在 2.4–上限 区间搜索长宽比。
- 轮廓长宽比上限走独立参数 `maxFootprintAspectNarrowLot`（default 4.0 / jp 4.5，代码为准）。
- 拓扑形态：枢纽（LDK）全宽置于入口端（z=0），服务房型照常嵌入；其余房间沿进深堆叠——公共房型贴枢纽（厨房保湿区）、私密房型在远端——由右侧纵向走廊串联；堆叠房间 ≤1 时省走廊直连枢纽；嵌入房间一律开门向宿主（走廊只覆盖枢纽顶边一段，band 的"顶角开向走廊"规则不适用）。

#### tanoji 落地细则（S4）

- **偏好拓扑**——田の字候选可行即用，全部不可行时回退 band 搜索（纯比分竞争会被 band 更大的可行域挤掉，故不混池比分）。
- 玄関格子生在中央走廊柱底端（宽=走廊宽，豁免最小净宽检查，日式 0.9–1.2m 玄関属正常）；够不着走廊的侧翼格子（如玄関旁的浴室）开门向玄関。
- 侧翼服务房型在下（近玄関）、私密房型在上（近 LDK）。

#### l_shape 落地细则（S5）

- **站点约束**——只出 L 候选。
- `LayoutFootprint` 扩展可选 `polygon`（轴对齐，width/depth 保持包围盒语义），消费方三处：validator 覆盖率/面积基数用真 L 面积、scene-executor 外墙检测（开窗/入户门）沿 L 边界、`footprintBoundary`/`longestExteriorEdge` 多边形感知。
- 主翼=单排 band（LDK 最左、其上叠侧翼，浴室/储物/玄关照常嵌入 LDK）；侧翼=私密房型堆叠，≥2 间时带内侧走廊（走廊底边开向 LDK），1 间时直连 LDK；翼宽比例在 0.35/0.45/0.55×W 中搜索。
...
不在策略层另建数值——**从 NormProfile 的面积档位表取**（[NORMS_PROFILE_DESIGN.md](NORMS_PROFILE_DESIGN.md) §2.3），含 LDK/DK 随卧室数的阶梯。策略层只做两件事：按 areaBand 在舒适区间内取 ideal 值；总面积不够装下全部 ideal 时按类型优先级压缩（卧室、LDK 保 ideal，走廊/储物先压到 min）。
...
## 5. 输入输出 schema（代码为准：`src/strategy.ts`）
...
按 §1 不变式 2，**尚无消费者的字段不进 schema**：`explicitWindowRooms` / `ensuiteBedroomCount`（暂无消费者）等有消费者再加。BriefFacts 用确定性关键词扫描而非扩展模型抽取 schema——不动 brief prompt，回归风险为零；尺寸/形状抽取（`detectSiteHint`/`detectNarrowLot`/`detectLShape` 三语词表）同样确定性，模型抽取版等确定性抽取在 eval 中暴露漏检时再评估。
...
**待办（线上验证债）**：S0–S5 的线上 eval 回归一次都没跑过（API 余额），恢复后至少跑 case-02/03/04/05/06/08/10/12/13/14（default 组 + JP 组），并为 tanoji / l_shape 补新 eval case。
