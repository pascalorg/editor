...
const BEDROOM: FurnitureRequirement[] = [
  ...
  single('bed', '床', ['bed', '双人床', '床'], /\bbed\b|(?<![头铺沙发]|床头)床(?!头|垫|品)/iu),
  single('wardrobe', '衣柜', ['wardrobe', '衣柜'], /衣柜|衣橱|wardrobe|closet/i),
]

const LIVING: FurnitureRequirement[] = [
  single('sofa', '沙发', ['sofa', '沙发'], /沙发|sofa|couch/i),
  single('coffee_table', '茶几', ['coffee table', '茶几'], /茶几|coffee[-_ ]?table/i),
]

const KITCHEN: FurnitureRequirement[] = [
  single('sink_counter', '水槽柜', ['kitchen sink', '水槽柜', '水槽'], /水槽|洗菜|\bsink\b/i),
  single('stove', '灶台', ['stove', 'cooktop', '灶台', '燃气灶'], /灶|炉(?!具架)|stove|cooktop|\brange\b/i),
  single('fridge', '冰箱', ['fridge', '冰箱'], /冰箱|fridge|refrigerator/i),
]

const BATHROOM: FurnitureRequirement[] = [
  single('toilet', '马桶', ['toilet', '马桶'], /马桶|坐便|toilet|\bwc\b/i),
  single('washbasin', '洗手台', ['bathroom vanity', 'basin', '洗手台', '浴室柜'], /洗手台|洗手盆|台盆|浴室柜|basin|vanity/i),
  {
    key: 'shower_or_bathtub',
    label: '淋浴或浴缸',
    options: [
      { label: '淋浴', searchTerms: ['shower', '淋浴房', '淋浴'], match: /淋浴|shower/i },
      { label: '浴缸', searchTerms: ['bathtub', '浴缸'], match: /浴缸|bathtub|\btub\b/i },
    ],
  },
]

const STUDY: FurnitureRequirement[] = [
  single('desk', '书桌', ['desk', '书桌', '办公桌'], /书桌|办公桌|写字台|desk/i),
  single('office_chair', '办公椅', ['office chair', '办公椅'], /办公椅|转椅|office[-_ ]?chair|desk[-_ ]?chair/i),
]
