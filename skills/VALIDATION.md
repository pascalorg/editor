...
| D02 automatic-routing baseline | Failed at `94/100` exact against the frozen `95/100` threshold; both positive classes were `25/25`, while furniture-adjacent and other-3D negatives were `23/25` and `21/25` |
| Post-merge Claude installation | `5/5` fresh isolated configurations installed bundle `0.1.2` from the exact merge commit; all 22 canonical skill and manifest files matched digest `c970e5df…9102` |
| Post-merge skills.sh installation | skills CLI `1.5.24` copied both skills into a clean project; all 18 skill files matched merged source digest `cbb4ee93…6843` |
| Post-merge Codex installation | Codex CLI `0.153.4` installed bundle `0.1.2` from the exact Git marketplace commit; all 22 canonical skill and manifest files matched digest `c970e5df…9102` |
...
- The single-run D02 automatic-routing baseline failed and was not rerun. It recorded two semantic false activations and four budget-limit invalid observations, leaving `96/100` valid; each skill's precision was `25/26`. Forty-six terminal cost receipts total `$6.7027235`; 54 early Skill captures have unknown cost rather than zero cost. Independent post-run audit remains pending. There is no automatic-routing promotion or improvement claim because no prior-version comparison ran.
