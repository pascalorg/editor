...
The candidate `furniture-fit/SKILL.md` SHA-256 is `623ed05f190752012d1bc7563ff762ffca613c8cb5d1e9a03cd8a788589e6301`. Package validation and strict plugin-manifest validation cover the source shape, bundled references, structured semantic next-action cases, version agreement, and marketplace manifests. No native agent-host task cohort, clean installation, publication, marketplace review, external adoption, or retention result is claimed for bundle 0.1.4; the skill metadata therefore records `package-checks-only`.
