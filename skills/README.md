...
Status on 2026-09-13. Installable and publicly listed are separate results.
...
The hosted server is optional. Configure `PASCAL_API_KEY` in the Cursor plugin dashboard for the intended account; for local-only work, disable `pascal-hosted` under **Customize → MCPs** to avoid its unauthenticated 401. The published 0.1.8 listing has no Cursor variable schema because it was parsed from the Claude manifest; a publisher refresh to the corrected `skills/.cursor-plugin/plugin.json` is required before that hosted configuration works. Do not paste a key into the cached plugin files.
