...
| `apps/editor/next.config.ts` (output) | Keep `output: 'standalone'` **unconditional**. Upstream gates it behind `PASCAL_PORTABLE_BUILD=1`; taking that produces a green build with no standalone directory to serve, and the deploy fails at the copy step rather than at the build. **Do NOT take `outputFileTracingRoot`** — this line used to say it was safe, and that was wrong; it cost a failed release. It decides where `.next/standalone` puts traced dependencies, and pointing it at the monorepo root moves them to `standalone/node_modules`, outside the single subtree `deploy-bundle.yml` copies. Build green, assemble green, server boots and logs normally, and every route needing a traced module then fails to resolve — `/api/health` answered nothing for a full minute with not one line in the log. Upstream ships the whole standalone tree; we ship one subtree of it. |
