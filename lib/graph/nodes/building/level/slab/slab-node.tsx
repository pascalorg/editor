...
/**
 * Slab node schema - extends BaseNodeSchema with slab-specific properties
 */
export const SlabNodeSchema = createNodeSchema(
  'slab',
  {},
  {
    withGrid: true,
...
// Register the schema
registerNodeSchema('slab', SlabNodeSchema)
    ...
    const handleGridClick = (e: GridEvent) => {
      ...
      if (slabStateRef.current.startPoint === null) {
        ...
        if (previewSlabId) {
          ...
          const slabNode = currentLevel?.children.find(
            (child: BaseNode) => child.id === previewSlabId,
          )
