...
Persistent and draft labels read `useViewer.unit` at render time. Node JSON always remains in metres; unit changes must not mutate scene data or create history entries. Resting, linked measurements render near-black; hovered, selected, and actively drafted measurements use the indigo product accent; dangling measurements remain red. Keep this hierarchy consistent between 2D geometry, 3D geometry, endpoints, fills, and value-label outlines.
...
The 3D hover reticle is a screen-sized, double-sided ring oriented to the resolved surface normal. Its short RGB axes stay in the level measurement frame, while a neutral stem communicates the surface normal. After the first point, full X/Y/Z guides pass through the active anchor; 2D mirrors this with an upright reticle and X/Z guides. The candidate axis brightens and thickens, then strengthens again when magnetically locked; inactive axes remain dashed and subdued. The 3D axis/surface targets use a white halo beneath their axis color and strengthen with the same candidate/locked state. A scene-proximity guide uses the same halo treatment, an `Align X/Y/Z` label, and, in 2D, a source beacon so the relationship remains legible over dense geometry.
