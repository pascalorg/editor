...
**Room detection is a commit-time job, not a per-tick one.** A wall gesture that reshapes rooms previews the automatic slabs/ceilings it bounds with `createWallBoundSurfaceFollower` (boundary membership read once at arm time, a few line intersections per tick). On drop it writes the walls in one `runAsSingleSceneHistoryStep`; the live space-detection sync reconciles sides, zones, slabs and ceilings inside that write, once and incrementally, and its writes join the step. Hold history with `beginSceneHistoryPauseSession(useScene, { gesture: movingNodeId })` and commit through `commitStep`, so the 2D overlay and the 3D tool share one pause and whichever view drops records exactly one step. Reference: the 3D `MoveWallTool`.
