...
Custom meshes use the reusable `MaterialRef` model through stable, user-named object slots. `CustomMeshNode.slots` maps slot IDs to `scene:` or `library:` references, `slotNames` stores their editable labels, and each `CustomMeshFace.materialSlot` stores one slot ID. `body` is the permanent base slot and the fallback for unbound or unresolved slots.
