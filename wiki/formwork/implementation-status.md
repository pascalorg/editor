...
3. **Nothing is validated.** P7 is still 0%, so a shutter can bill cleanly and still clash — ties through rebar, props onto a slab that cannot carry them, panels through an intersection. The parts model made the bill possible; it did not make it safe, and the parts table saying "1 part beyond capacity" is a per-member check, not the 20-line invariant suite.

**The fair characterisation: the engine is roughly two-thirds there, the product surface now a little over half.** Everything that *computes* — pressure, the beam core, both design chains, coverage, layout, measurement, and now the parts — is largely done and tested. What is still missing on the presenting side is export and audit rather than legibility: no CSV, no cut sheet, no validation report, no project rollup. Three things have now landed and they compound: the design report made the engine legible, the settings panel made it governable, and the parts model made it *addressable* — the walers the report describes at 300 mm centres are nine objects with marks, each of which can be clicked, priced, substituted or left off the order. That is the difference between a calculation someone reads and a shutter someone orders.
...
Core: 1235 tests passing across 92 files (18 formwork files, 469 formwork tests). Nodes: 1051 passing across 121 files, including 12 in `formwork-assembly/floorplan.test.ts` — among them the two sign checks that catch a shutter drawn inside its own pour or against the wrong face of a wall, both of which are invisible on a symmetric test wall. Repo: 3257 passing across 358 files.
...
**Re-solve the stale assemblies after a settings change.** This was deliberately queued behind the parts model, and the reason has now arrived: what a re-solve has to preserve is exactly the per-part overrides that did not exist before. Today a settings change reports how many assemblies it invalidated and asks the user to re-run `attach_formwork` by hand, which is an odd thing for an application to ask — and it is now also lossy in a way it was not last week, because a hand re-attach discards the substitutions and omissions somebody recorded against marks. The work is small and the payoff is visible: change the concrete mix, and every shutter on the job re-solves with its decisions intact and its stale ones named.

Two obligations that are worth stating before the code is written, because they are the same two the parts model had.

**The overrides survive by mark, and the report has to say what did not.** A wall that grows keeps every decision whose part it still produces; a wall that shrinks below a panel does not, and `orphanedOverrides` already reports those. What must not happen is a silent re-solve that drops three decisions and says "12 assemblies updated". The part inspector's stale-edits section is the pattern — keep the orphan, name it, and let a person say it is finished with.

**A re-solve is not a re-attach.** `attach_formwork` decides *how many* assemblies a host gets from its pour units, and a settings change usually does not move that. Re-solving means rebuilding the geometry of the assemblies that exist, not deleting and recreating nodes — recreating them changes their ids, which orphans nothing visible but does break undo grouping and any selection pointing at one.

After that, the two rows that now have their prerequisite: **CSV export** (`bomLines` is already the row model, so this is a serialiser and a download, not new engine work) and **P7 validation**, which is the one that changes what the feature is allowed to claim. A bill that totals correctly and a shutter that stands up are different assertions, and only the first is currently made.
