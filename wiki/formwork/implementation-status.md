...
| **P4** Quantities, BOM, exports | 31% | Floorplan + schedule + settings panel done; no BOM, no CSV, no project rollup |
...
| **P8** AI + drawings | 13% | 7 chat tools against the plan's ~25; no MCP formwork tools |

Counting partials as half: **19 done, 7 partial, 16 not started → 51%**. The solver pipeline scores separately at **5 of 13 phases done, 4 partial → 58%**.
...
**The fair characterisation: the engine is roughly two-thirds there, the product surface roughly a third.** Everything that *computes* — pressure, the beam core, both design chains, coverage, layout, measurement — is largely done and tested. Everything that *presents or exports* what is computed is mostly not: no BOM, no cut sheet, no validation report, no parts table, AI parity around 28%. Two of those have now landed and they are a pair: the design report made the engine legible, and the settings panel made it governable. The chain can be read on screen rather than taken on faith, and the assumptions it reads can be replaced with what the job actually stated — which is what turns `95.6 kN/m²` from a number into a claim someone is willing to sign.
...
Core: 1193 tests passing across 91 files (17 formwork files, 427 formwork tests). Nodes: 1016 passing across 119 files, including 12 in `formwork-assembly/floorplan.test.ts` — among them the two sign checks that catch a shutter drawn inside its own pour or against the wrong face of a wall, both of which are invisible on a symmetric test wall. Repo: 3164 passing across 354 files.
...
**The parts model** (plan §5.3) — the one structural gap with the most downstream weight. It blocks the parts table, the BOM, the cut sheet, and "click a waler and read its own utilisation", which is the natural next step from a report that currently speaks per member *type* rather than per part.

The design report closed the "is the engine legible" question and the settings panel closed "can the user govern what it assumed". What is still not addressable is a *thing*: the report says walers at 300 mm and the 3D shows nine of them, and there is no object between those two statements to select, mark, count, price or cut. Every remaining P4/P5/P7 row consumes that object, which is why it is worth building before any of them rather than alongside.

Two obligations that will otherwise be discovered late. Marks have to be **deterministic** — a mark that changes when the scene is re-solved is a mark that cannot appear on a drawing sent to site, so it must derive from position and kind rather than from iteration order or a counter. And a part is a *result*, not stored state: the settings node's rule holds here too, so parts are recomputed like the spacings they come from and the scene stores only what the user overrode.

Then the settings work suggests its own follow-on: `formworkSettings()` now has a panel and a tool pair, but nothing has closed the loop it opens — a settings change reports how many assemblies it invalidated and asks the user to re-run `attach_formwork` by hand. Re-solving the stale assemblies is a small piece of work with a visible payoff, and it belongs after the parts model rather than before, because what a re-solve should preserve is exactly the per-part overrides that do not exist yet.
