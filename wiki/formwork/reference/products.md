...
**Tie design rules (extracted from the manual's text layer, 2026-08-15):** ✅
- Anchor holes are **Ø20 mm** on the Xlife face, with an integrated anchor protection (page 17).
- **Tie count per pour height on a 2.70 m panel** (no overlap): only **2 anchorages in height** up to a 2.70 m pour; 3.00 m panels use 3. The tie positions read off the anchoring-situation drawings as ~**90 cm and 120–150 cm** offsets from the panel edges (page 26): 1.20/1.50 m panels anchor at 120–150 cm; 2.70 m panels at 90 + 120–150; 3.00 m at 120–150 + 120–150. These are dimension-annotation fragments, not a full level chain.
- Anchoring hardware (from the PT component overview, page 85): tie rod 15.0 mm galv. **0.50–7.50 m** (`581821000`–`581882000`, 0.72–10.7 kg), super nut 15.0 `581966000` 1.1 kg, wing nut `581961000` 0.31 kg, angle anchor plate 12/18 `581934000` 1.5 kg, hex nut 15.0 `581964000` 0.23 kg, **Frami pressure plate 8/9** `588466000` 0.55 kg, spacer 20 cm `581907000` 0.04 kg.

❌ Still missing for a catalog seed: the tie-hole **dimension chains** per panel (the exact vertical levels the frame is drilled at) and the tie/anchor load capacities — both sit in the manual's dimensioned drawings, which the PDF text layer does not carry (the attempt to extract them 2026-08-15 recovered only the fragments above). Do not seed the panel set until those are read from the drawings, because the layout places ties only where a panel's `tieHoles` say the frame was drilled. If the full chain is ever needed without the drawing, Doka's item list + handbook combo is the source: item list `direct.doka.com/_ext/downloads/itemlists/en/189962.pdf`, manual `999806241_2014_09_online`.
